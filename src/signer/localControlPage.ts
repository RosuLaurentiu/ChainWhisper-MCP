...
const actionPresentation = (
  ...
  const known: Record<string, { title: string; button: string }> = {
    confirmation_form_diagnostic: {
      title: 'Test signer confirmation',
      button: 'Confirm test',
    },
    create_trade: {
    ...
    create_recurring: {
      ...
      button: 'Confirm complete recurring order',
    },
    fill: {
    ...
    onboard_privacy: {
      title: 'Enable wallet privacy',
      button: 'Confirm privacy onboarding',
    },
    enable_private_token: {
      title: 'Enable private token account',
      button: 'Confirm private token setup',
    },
    send_order_message: {
...
const privacyLabel = (request: ConfirmationRequest): string => {
  ...
  const content = JSON.stringify({
    ...
    details: request.details?.map(({ label }) => label),
    amounts: request.amounts,
  }).toLowerCase();
  return content.includes('private') || content.includes('hidden')
    ? 'Privacy-protected'
    : 'Public terms';
};
...
const renderAutonomyEditor = (
  ...
  const amountFields = (
    ...
    entries.length
      ? `<fieldset class="policy-fields"><legend>${escapeHtml(title)}</legend>${entries
          .map(
            ({ asset, amount }, index) =>
              `<label>${escapeHtml(asset)} base units<input name="autonomy.${prefix}.${index}" inputmode="numeric" pattern="(?:0|[1-9][0-9]*)" value="${escapeHtml(amount)}" required></label>`,
          )
          .join('')}</fieldset>`
      : '';
  const limitFields = [
    ['maximumNativeValuePerAction', 'Maximum native value per action', editor.maximumNativeValuePerAction],
    ['maximumNativeValueCumulative', 'Maximum native value cumulative', editor.maximumNativeValueCumulative],
    ['maximumNetworkFeePerAction', 'Maximum network fee per action', editor.maximumNetworkFeePerAction],
    ['maximumNetworkFeeCumulative', 'Maximum network fee cumulative', editor.maximumNetworkFeeCumulative],
  ...
  const priceBands = editor.priceBands.length
    ? `<fieldset class="policy-fields policy-wide"><legend>Price bands (buy units / sell units)</legend>${editor.priceBands
        .map(
          (band, index) =>
            `<div class="price-band">
              <strong>${escapeHtml(band.sellAsset)} → ${escapeHtml(band.buyAsset)}</strong>
  ...
  return `<section class="policy-editor" aria-labelledby="policy-editor-title">
    <div>
      ...
      <p class="muted">You may lower budgets, tighten price bands, reduce counts, shorten expiry, or disable agent-visible private amounts. Agent Control will reject any broader edit.</p>
    </div>
    <div class="policy-grid">
      <label class="policy-wide">Expiry (ISO 8601)<input name="autonomy.expiresAt" value="${escapeHtml(editor.expiresAt)}" required></label>
      <label class="ack policy-wide"><input type="checkbox" name="autonomy.agentVisiblePrivateAmounts" value="true"${editor.agentVisiblePrivateAmounts ? ' checked' : ''}> Allow this agent to choose private amounts under this policy.</label>
...
const renderConfirmation = (
  ...
  const presentation = actionPresentation(request.action);
  const primaryRows = (request.details ?? []).filter(({ label }) => {
    const normalized = label.toLowerCase();
    return normalized.includes('you send') || normalized.includes('you receive');
  });
  const otherRows = (request.details ?? []).filter(
    (row) => !primaryRows.includes(row),
  );
  ...
  const counterparty = request.counterparty
    ? `<div><span>Counterparty</span><strong class="mono">${escapeHtml(shortAddress(request.counterparty))}</strong></div>`
    : '';
  const step =
  ...
  return `<section class="approval-card" aria-labelledby="approval-title">
    ...
    <div class="stats" aria-label="Action costs and destination">
      ...
      <div><span>Native value</span><strong>${escapeHtml(request.nativeValue)}</strong></div>
      <div><span>Gas limit</span><strong>${escapeHtml(request.gasCap)}</strong></div>
      ${counterparty}
    </div>
    ${details}
    <div class="result">
...
const renderPrivateValues = (
  ...
  return `<section class="approval-card" aria-labelledby="private-title">
    ...
    <p class="lead">These values go directly to the signer on this device. They are never sent through the agent conversation.</p>
  ...
  return `<section class="wallet-setup" aria-labelledby="wallet-setup-title">
    ...
    <div class="setup-grid">
      <form method="post" action="/action" class="setup-card">
        ...
        <label for="wallet-env-path">Signer .env file</label>
        <input id="wallet-env-path" name="environmentFilePath" value="${path}" required autocomplete="off" spellcheck="false">
      ...
      <form method="post" action="/action" class="setup-card">
        ...
        <label for="generated-env-path">Signer .env file</label>
        <input id="generated-env-path" name="environmentFilePath" value="${path}" required autocomplete="off" spellcheck="false">
        ${replacing ? '<label class="ack"><input type="checkbox" name="confirmReplacement" value="replace-agent-wallet" required> I approve replacing the current Agent Wallet after restart.</label>' : ''}
        <button class="button button-secondary" name="action" value="generate-wallet">Create new Agent Wallet</button>
      </form>
    </div>
    <p class="muted compact">After restart, fund the displayed address with COTI for network fees and only the tokens you want the agent to use. Privacy onboarding is completed from this page after funding.</p>
  </section>`;
};
...
const renderControlSummary = (
  ...
  const operations = summary.recentOperations?.length
    ? summary.recentOperations
        .map((operation) => {
          ...
          return `<li>
            <span><strong>${escapeHtml(operation.label)}</strong><small>${escapeHtml(operation.status)}</small></span>
  ...
  const privacyControl = actions?.onboardPrivacy
    ? `<form method="post" action="/action" class="privacy-control">
        <input type="hidden" name="csrf" value="${escapeHtml(csrfToken)}">
        <input type="hidden" name="intent" value="control">
        <button class="button button-primary" name="action" value="onboard-privacy">Set up wallet privacy</button>
      </form>`
    : '';
